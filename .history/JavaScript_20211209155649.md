...
* Symbol值不能与其他类型的值进行运算，可以显示转为字符串，可以转为布尔值，但是不能转为数值
```js
let s = Symbol()
...
* 新生代垃圾回收 scavenge算法
  ...
  * 解析二
  ...
  4. 当From中所有的存活对象全部复制完毕之后, From和To就会对调, 也就是From被闲置, To在使用;
    如此循环
...
window包括 document lo cation navigation screen history

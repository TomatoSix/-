...
13. 使用 setTimeout 实现 setInterval 方法

    ```js
    function mysetinterval(fn, time) {
      console.log("利用steTimeout实现setInterval");
      function interval() {
        //执行该函数，异步被挂起time时间后在执行，一上来就执行fn
...
38. 实现 `a == 1 && a == 2 && a == 3`和`a === 1 && a === 2 && a === 3`
...
46. setTimeout 模拟实现 setInterval

47. 将虚拟 DOM 转化为真实 DOM

48. 实现一个对象的 flatten 方法

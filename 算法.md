...
10. 删除链表的倒数第 N 个节点(中)-需要设置虚拟节点
    ...
    var removeNthFromEnd = function (head, n) {
      // 必须设置虚拟节点， 否则不能解决要删除的节点是头结点的情况
      let dummyNode = new ListNode(0, head);
      let fast = dummyNode,
        slow = dummyNode;
      while (n--) fast = fast.next;
      // 只有fast.next不为空， fast才能移动到最后一个节点
      // 如果只是fast不为空， 下一步fast = fast.next， fast最终会指向null
...
1. 深度优先遍历是每条路都必须走， 例如题目`二叉树的所有路径`
2. 回溯也是每条路都必须走，但必须判断该条路是否正确， 不正确则必须回溯, 例如题目`二叉树中和为某一值的路径` `向下的路径节点之和`
...
11. 树的子结构(中等)
    ...
    var isSubStructure = function (A, B) {
      ...
      // if (!A || !B) return false;
      // 或者
      if (!B) return false; // 如果B是空树的话，肯定是错误的
      if (!A) return false; // 存在B，不存在A，则两个不相等
      return (
        isSameTree(A, B) ||
    ...
    var isSameTree = function (A, B) {
      // 下面两行顺序不能换
      // 1. 只要B为空， 肯定是A的子结构， 返回true
      // 2. 若A为空直接返回false, 不能解决A和B都为空的情况
      if (!B) return true;
      // 解决 B存在， A不存在的情况
      if (!A) return false;
...
7. 二叉搜索树的最近公共祖先
   ...
   var lowestCommonAncestor = function (root, p, q) {
     if (!root) return null;
     // root.val比p,q节点都大, 则祖先一定在root.left左子树
     if (root.val > p.val && root.val > q.val) {
       let left = lowestCommonAncestor(root.left, p, q);
       return left;
     }
     // root.val比p,q节点都小，则公共祖先一定在root.right右子树
     if (root.val < p.val && root.val < q.val) {
       let right = lowestCommonAncestor(root.right, p, q);
       return right;
     }
...
25. 最长回文子序列
    ...
    const longestPalindromeSubseq = (s) => {
      ...
      for (let i = strLen - 1; i >= 0; i--) {
        // 为什么从i+1开始， 因为已经设计dp[i][i]=1
        // 因为是从内向外, 先从s[i]和s[i+1]开始
        for (let j = i + 1; j < strLen; j++) {
          if (s[i] === s[j]) {
            // 回文假设从里向外扩散
            // 由下面的递推公式得出i是倒序遍历，j是正序遍历
            dp[i][j] = dp[i + 1][j - 1] + 2;
          } else {
            dp[i][j] = Math.max(dp[i + 1][j], dp[i][j - 1]);
          }
    ...
    var exist = function (board, word) {
      ...
      const dfs = function (r, c, index) {
        ...
        if (r < 0 || r >= m || c < 0 || c >= n || board[r][c] !== word[index])
          return false;
        // 索引等于单词长度-1，说明全匹配上了
        // 因为前面board[r][c] !== word[index] 已经判断字符是否相等了
        if (index === word.length - 1) return true;
        // 保存当前字符
        let temp = board[r][c];
        // 将当前字符设置为空，防止四个方向dfs再次遍历到， 因为同一个单元格内的字母不允许被重复使用
        board[r][c] = "";
        // 要用或者
        let res =
          dfs(r + 1, c, index + 1) ||
...
16. 打印从 1 到最大的 n 位数-考虑大数(1. 回溯 2. 位运算)(难)
   ...
   var findRedundantConnection = function (edges) {
     ...
     for (let i = 0; i < len; i++) {
       ...
       if (find(parent, node1) !== find(parent, node2)) {
         // 两个代表节点不相同，说明之前两个节点之间不连通，合并
         union(parent, node1, node2);
       } else {
         // 两个代表节点相同，说明在遍历当前边之前，两个顶点之间已经连通，当前边会导致环出现
         return edge;
       }
     }
     return [0];
...
9. 机器人的运动范围
   ...
   var movingCount = function (m, n, k) {
     ...
     function dfs(x, y) {
       ...
       visited[x][y] = true;
       // 该条件不能放在前面的if判断中，不然无法将已走过的但是sum>k的情况遍历为true
       if (sum(x) + sum(y) <= k) {
         res++;
         dfs(x + 1, y);
         dfs(x - 1, y);
...
3. 最长回文子串(中)
   ...
   var longestPalindrome = function (s) {
     ...
     function helper(m, n) {
       ...
       if (n - m - 1 > res.length) {
         // 这里必须用slice或者substring, 都是表示从开始索引到结束索引
         res = s.slice(m + 1, n);
       }
     }
     return res;
...
1. 寻找图中是否存在路径
   ...
   var validPath = function (n, edges, source, destination) {
     ...
     for (let i = 0; i < edges.length; i++) {
       let node1 = edges[i][0];
       let node2 = edges[i][1];
       // 如果节点node1和node2连通的话，它们一定拥有相同的根节点
       // 两个代表节点不相同，说明之前两个节点之间不连通，将其合并
       if (find(parent, node1) !== find(parent, node2)) {
         union(parent, node1, node2);
       }
     }
     // 如果两个根节点相同，说明两个顶点已经连通
...
6. 打印从 1 到最大的 n 位数-考虑大数(1. 回溯 2. 位运算)(难)
   https://leetcode-cn.com/problems/da-yin-cong-1dao-zui-da-de-nwei-shu-lcof/solution/4chong-jie-fa-bao-li-zi-fu-chuan-wei-yun-v2dr/
...
10. 最小的 k 个数(敲重点， 利用快排) - 见 数组中的第 k 大的数字(难)
...
25. 数组中的第 k 大的数字
    ...
    function partition(nums, left, right) {
      ...
      for (let i = left; i < right; i++) {
        // 逐个考察元素，和 pivot 比较
        if (nums[i] < pivot) {
          // 如果当前元素比 pivot 小, 将它交换到 pivotIndex 的位置
          // 或者[arr[pivotIndex], arr[i]] = [arr[i], arr[pivotIndex]]
          swap(nums, i, pivotIndex);
          // 每找到一个比pivot小的元素, pivotIndex+1, 所以pivotIndex左边都是比pivot小的
          pivotIndex++;
        }
      } // 循环结束时，pivotIndex左边都是比pivot小的
      // 或者 [arr[pivotIndex], arr[right]] = [arr[right], arr[[pivotIndex]]]

...
19. Event Loop
    ...
    列，主代码执行完毕，调用栈空闲，开始进行事件循环首先进入 timers 阶段，执行 timer1 的回调函数，打印 timer1，并将 promise1.then 回调放入 microtask 队列，同样的步骤执行 timer2，打印 timer2；至此，timer 阶段执行结束，event loop 进入下一个阶段之前，执行 microtask 队列的所有任务，依次打印 promise1、promise2。
...
30. js获取，更新，添加，删除dom操作
    ...
    document.getElementsByName();  //名称选择器
...
31. js垃圾回收机制
    ...
    分代收集:
        ...
        (3). 两种不同的对象对应不同的回收策略，老生代，V8把堆分成新生代和老生代两个区域，新生代中存放临时对象中存放持久对象并且让副垃圾回收器，主垃圾回收器分别负责新生代和老生代的垃圾回收，这样就可以高效的垃圾回收
...
4. 作用域问题
    ...
    var z
    //先执行y=1,因为这里的y在声明时没有使用var，默认为全局变量，然后将一个全局变量的值赋值给局部变量x
